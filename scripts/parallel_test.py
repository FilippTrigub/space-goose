...
class ParallelMetrics:
    ...
    def get_summary(self) -> Dict:
        ...
        deleted_project_ids = {r['project_id'] for r in successful_deletes if r.get('project_id')}
        overall_successes = len(successful_project_ids.intersection(deleted_project_ids))
        ...
        return {
            ...
            "delete_success_rate": (self.delete_successes / len(self.delete_results)) * 100 if self.delete_results else 0,
            "create_timeout_rate": (self.create_timeouts / total_projects) * 100,
            "delete_timeout_rate": (self.delete_timeouts / len(self.delete_results)) * 100 if self.delete_results else 0,
            "timing": {
                "create": {
...
class ParallelTester:
    ...
    async def create_project(self, session: aiohttp.ClientSession, project_name: str, project_index: int, timeout: int = 150) -> Dict:
        """
    ...
    async def delete_project(self, session: aiohttp.ClientSession, project_id: str, project_name: str, timeout: int = 30) -> Dict:
        """
    ...
    async def run_create_phase(self, session: aiohttp.ClientSession, num_projects: int, create_timeout: int) -> List[Dict]:
        """
        ...
        project_names = [f"perf-test-par-{i}-{timestamp}" for i in range(1, num_projects + 1)]
        ...
        print(f"📊 Success: {len(successful_creates)}/{num_projects} ({len(successful_creates)/num_projects*100:.1f}%)")
        print(f"❌ Failed: {len(failed_creates)} projects")

        return processed_results

    async def run_delete_phase(self, session: aiohttp.ClientSession, create_results: List[Dict], delete_timeout: int) -> List[Dict]:
        """
        Delete all successfully created projects simultaneously
        """
        successful_creates = [r for r in create_results if r['success'] and r['project_id']]
        ...
        delete_tasks = [
            self.delete_project(session, r['project_id'], r['project_name'], delete_timeout)
        ...
        print(f"📊 Success: {len(successful_deletes)}/{len(successful_creates)} ({len(successful_deletes)/len(successful_creates)*100:.1f}%)")
        print(f"❌ Failed: {len(failed_deletes)} projects")

        return processed_results

    async def run_test(self, num_projects: int, create_timeout: int = 150, delete_timeout: int = 30):
        """
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=100)) as session:
            ...
            create_results = await self.run_create_phase(session, num_projects, create_timeout)
            
            # Wait a moment between phases
            await asyncio.sleep(1)
            
            # Phase 2: Delete projects in parallel
            delete_results = await self.run_delete_phase(session, create_results, delete_timeout)
    ...
    def print_summary(self, summary: Dict):
        ...
        print(f"-"*40)
        print(f"Create: avg={create_timing['avg']:.1f}s, max={create_timing['max']:.1f}s, min={create_timing['min']:.1f}s ({create_timing['count']} successful)")
        print(f"Delete: avg={delete_timing['avg']:.1f}s, max={delete_timing['max']:.1f}s, min={delete_timing['min']:.1f}s ({delete_timing['count']} successful)")
...
def main():
    ...
    parser.add_argument('--projects', '-n', type=int, default=5, 
