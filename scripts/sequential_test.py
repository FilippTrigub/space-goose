...
class PerformanceMetrics:
    ...
    def get_summary(self) -> Dict:
        ...
        create_times = [
            i["create_time"] for i in self.iterations if i["create_success"]
        ]
        delete_times = [
            i["delete_time"] for i in self.iterations if i["delete_success"]
        ]
        ...
        return {
            ...
            "timing": {
                ...
                "avg_delete_time": (
                    sum(delete_times) / len(delete_times) if delete_times else 0
                ),
...
class SequentialTester:
    ...
    async def create_project(
        self, session: aiohttp.ClientSession, project_name: str, timeout: int = 150
        ...
        try:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(
                        f"❌ Create failed with status {response.status}: {error_text}"
                    )
    ...
    async def delete_project(
        self, session: aiohttp.ClientSession, project_id: str, timeout: int = 30
        ...
        try:
            async with session.delete(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    print(
                        f"❌ Delete failed with status {response.status}: {error_text}"
                    )
    ...
    async def run_iteration(
        ...
        # Create project
        create_success, project_id, create_time, create_timeout_occurred = (
            await self.create_project(session, project_name, create_timeout)
        )
        ...
        if create_success and project_id:
            ...
            delete_success, delete_time, delete_timeout_occurred = (
                await self.delete_project(session, project_id, delete_timeout)
            )
            if delete_success:
    ...
    async def run_test(
        self, iterations: int, create_timeout: int = 150, delete_timeout: int = 30
    ):
        """
            ...
            for i in range(1, iterations + 1):
                try:
                    result = await self.run_iteration(
                        session, i, create_timeout, delete_timeout
                    )
                    ...
                    print(
                        f"📈 Progress: {i}/{iterations} ({success_rate:.1f}% success rate)"
                    )
    ...
    def print_summary(self, summary: Dict):
        ...
        print(f"-" * 30)
        print(
            f"Create Times: avg={timing['avg_create_time']:.1f}s, max={timing['max_create_time']:.1f}s, min={timing['min_create_time']:.1f}s"
        )
        print(
            f"Delete Times: avg={timing['avg_delete_time']:.1f}s, max={timing['max_delete_time']:.1f}s, min={timing['min_delete_time']:.1f}s"
        )
        print(
            f"Total Times:  avg={timing['avg_total_time']:.1f}s, max={timing['max_total_time']:.1f}s, min={timing['min_total_time']:.1f}s"
        )
...
def main():
    ...
    try:
        summary = asyncio.run(
            tester.run_test(args.iterations, args.create_timeout, args.delete_timeout)
        )
        ...
        if args.output:
            with open(args.output, "w") as f:
                json.dump(
                    {"summary": summary, "iterations": tester.metrics.iterations},
                    f,
                    indent=2,
                )
